...
public unsafe class RandomX : IHashAlgorithm, IDisposable
{
    ...
    protected virtual void Dispose(bool disposing)
    {
        ...
        if (disposing)
        {
            // Libérez les ressources managées ici
            Console.WriteLine($"elva Debug ScashJob -----> Dispose: Libération des ressources managées.");
        }

        // Libérez les ressources non managées ici
        lock (realms)
        {
            foreach (var realm in realms.Values)
            {
                foreach (var seed in realm.Values)
                {
                    var (_, seedVms) = seed;
                    foreach (var vm in seedVms)
                    {
                        vm?.Dispose();
                    }
                }
        ...
        Console.WriteLine($"elva Debug ScashJob -----> Dispose: Libération des ressources non managées terminée.");
