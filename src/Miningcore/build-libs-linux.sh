...
((cd /tmp && rm -rf RandomARQ && git clone https://github.com/arqma/RandomARQ && cd RandomARQ && git checkout 14850620439045b319fa6398f5a164715c4a66ce && mkdir build && cd build && cmake -DARCH=native .. && make) && (cd ../Native/librandomarq0 && cp /tmp/RandomARQ/build/librandomx.a . && make clean && make) && mv ../Native/librandomarq0/librandomarq.so "$OutDir")

cp ./Blockchain/Hoosat/libhoohash.so "$OutDir"