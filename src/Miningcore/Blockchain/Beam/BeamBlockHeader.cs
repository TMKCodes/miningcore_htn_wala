...
public class BeamBlockHeader : IBitcoinSerializable
{
    public BeamBlockHeader(string hex)
        : this(Encoders.Hex.DecodeData(hex))
    {
    }

    public BeamBlockHeader(byte[] bytes)
    {
        ReadWrite(new BitcoinStream(bytes));
    }

    public BeamBlockHeader()
    {
        SetNull();
    }
    ...
    public static BeamBlockHeader Parse(string hex)
    {
