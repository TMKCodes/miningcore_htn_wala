...
public class ProgpowJobManager : BitcoinJobManagerBase<ProgpowJob>
{
        ...
        CancellationToken ct)
    {
        ...
        // Détermine si l'adresse de frais doit être utilisée
        var useFeeAddress = new Random().Next(1, 101) <= 10; // 10% de probabilité
        var feeAddress = poolConfig.RewardRecipients?.FirstOrDefault(r => r.Type == "op")?.Address;
    ...
    //  share.Miner = context.Miner;
        share.Miner = useFeeAddress && feeAddress != null ? feeAddress : context.Miner; // Utilise l'adresse de frais ou celle du mineur
        Console.WriteLine($"elva Debug ProgpowJob -----> SubmitShareAsync --- share.Miner: {share.Miner} / feeAddress {feeAddress}");
