...
public class ScashPayoutHandler : PayoutHandlerBase,
    IPayoutHandler
{
    public ScashPayoutHandler(
    ...
    protected ScashDaemonEndpointConfigExtra extraPoolEndpointConfig;
    ...
    protected override string LogCategory => "Scash Payout Handler";
    ...
    public virtual Task ConfigureAsync(ClusterConfig cc, PoolConfig pc, CancellationToken ct)
    {
        ...
        extraPoolEndpointConfig = pc.Extra.SafeExtensionDataAs<ScashDaemonEndpointConfigExtra>();
        ...
        if(coin is ScashTemplate ScashTemplate)
        {
            minConfirmations = extraPoolEndpointConfig?.MinimumConfirmations ?? ScashTemplate.CoinbaseMinConfimations ?? ScashConstants.CoinbaseMinConfimations;
            payoutDecimalPlaces = ScashTemplate.PayoutDecimalPlaces ?? 4;
        }
        else
            minConfirmations = extraPoolEndpointConfig?.MinimumConfirmations ?? ScashConstants.CoinbaseMinConfimations;

        logger = LogUtil.GetPoolScopedLogger(typeof(ScashPayoutHandler), pc);
    ...
    public virtual async Task<Block[]> ClassifyBlocksAsync(IMiningPool pool, Block[] blocks, CancellationToken ct)
    {
        ...
        for(var i = 0; i < pageCount; i++)
        {
            ...
            var batch = page.Select(block => new RpcRequest(ScashCommands.GetTransaction,
    ...
    public virtual async Task PayoutAsync(IMiningPool pool, Balance[] balances, CancellationToken ct)
    {
        ...
        if(!(extraPoolConfig?.HasBrokenSendMany == true || poolConfig.Template is ScashTemplate { HasBrokenSendMany: true }))
        {
            if(extraPoolPaymentProcessingConfig?.MinersPayTxFees == true)
            {
                var subtractFeesFrom = amounts.Keys.ToArray();

                if(!poolConfig.Template.As<ScashTemplate>().HasMasterNodes)
                {
                    args = new object[]
                    {
            ...
            // send command
            tryTransfer:
            var result = await rpcClient.ExecuteAsync<string>(logger, ScashCommands.SendMany, ct, args);
            ...
            if(result.Error == null)
            {
                if(didUnlockWallet)
                {
                    ...
                    await rpcClient.ExecuteAsync<JToken>(logger, ScashCommands.WalletLock, ct);
                }
                ...
                if(string.IsNullOrEmpty(txId))
                    logger.Error(() => $"[{LogCategory}] {ScashCommands.SendMany} did not return a transaction id!");
                else
            ...
            {
                if(result.Error.Code == (int) ScashRPCErrorCode.RPC_WALLET_UNLOCK_NEEDED && !didUnlockWallet)
                {
                    if(!string.IsNullOrEmpty(extraPoolPaymentProcessingConfig?.WalletPassword))
                    {
                        logger.Info(() => $"[{LogCategory}] Unlocking wallet");

                        var unlockResult = await rpcClient.ExecuteAsync<JToken>(logger, ScashCommands.WalletPassphrase, ct, new[]
                        {
                        ...
                        if(unlockResult.Error == null)
                        {
                            ...
                            logger.Error(() => $"[{LogCategory}] {ScashCommands.WalletPassphrase} returned error: {result.Error.Message} code {result.Error.Code}");
                    }
                ...
                {
                    logger.Error(() => $"[{LogCategory}] {ScashCommands.SendMany} returned error: {result.Error.Message} code {result.Error.Code}");

                    NotifyPayoutFailure(poolConfig.Id, balances, $"{ScashCommands.SendMany} returned error: {result.Error.Message} code {result.Error.Code}", null);
                }
            }
        }
        ...
        else
        {
            ...
            await Parallel.ForEachAsync(amounts, parallelOptions, async (x, _ct) =>
            {
                ...
                await Guard(async () =>
                {
                    ...
                    var result = await rpcClient.ExecuteAsync<string>(logger, ScashCommands.SendToAddress, ct, new object[]
                    {
                        address,
                        amount,
                    });
                    ...
                    if(result.Error != null)
                        throw new Exception($"[{transferId}] {ScashCommands.SendToAddress} returned error: {result.Error.Message} code {result.Error.Code}");

                    if(string.IsNullOrEmpty(txId))
                        throw new Exception($"[{transferId}] {ScashCommands.SendToAddress} did not return a transaction id!");
                    else
