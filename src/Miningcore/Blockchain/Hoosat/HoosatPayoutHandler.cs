        ...
        for (var i = 0; i < pageCount; i++)
        {
            ...
            foreach (var block in page)
            {
                ...
                uint totalDuplicateBlockBefore = await cf.Run(con =>
                    blockRepo.GetPoolDuplicateBlockBeforeCountByPoolHeightAndHashNoTypeAndStatusAsync(con, poolConfig.Id,
                        Convert.ToInt64(block.BlockHeight), block.Hash, new[]
                        {
                ...
                {
                    MarkBlockAsOrphaned(block, poolConfig.Id, coin, blockRepo);
                    continue;
                }
                ...
                if (blockReward > 0)
                {
                    FinalizeBlockProcessing(block, poolConfig.Id, coin, blockRepo, blockInfo);
                }
                else
                {
                    MarkBlockAsOrphaned(block, poolConfig.Id, coin, blockRepo);
