...
public (Share Share, string BlobHex) ProcessShare(string nonce, uint workerExtraNonce, string workerHash, StratumConnection worker)
{
    Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> Début du traitement pour le worker: {worker.ConnectionId}");
    ...
    // Validation du nonce
    Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> Validation du nonce: {nonce}");
    if (!CryptonoteConstants.RegexValidNonce.IsMatch(nonce))
    {
        Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> ERREUR : Nonce malformé");
        throw new StratumException(StratumError.MinusOne, "malformed nonce");
    }

    // Clonage du template
    Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> Clonage du template");
    Span<byte> blob = stackalloc byte[blobTemplate.Length];
    blobTemplate.CopyTo(blob);

    // Injection d'ExtraNonce
    Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> Injection d'ExtraNonce: {workerExtraNonce}");
    var bytes = BitConverter.GetBytes(workerExtraNonce.ToBigEndian());
    bytes.CopyTo(blob[BlockTemplate.ReservedOffset..]);

    // Injection du nonce
    Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> Injection du nonce: {nonce}");
    bytes = nonce.HexToByteArray();
    bytes.CopyTo(blob[CryptonoteConstants.BlobNonceOffset..]);

    // Conversion du blob
    Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> Conversion du blob");
    var blobConverted = CryptonoteBindings.ConvertBlob(blob, blobTemplate.Length, blobType);
    if (blobConverted == null)
    {
        Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> ERREUR : Blob malformé");
        throw new StratumException(StratumError.MinusOne, "malformed blob");
    }

    // Calcul du hash d'en-tête
    Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> Calcul du hash d'en-tête");
    Span<byte> headerHash = stackalloc byte[32];
    hashFunc(RandomXRealm, BlockTemplate.SeedHash, blobConverted, headerHash, BlockTemplate.Height);

    var headerHashString = headerHash.ToHexString();
    Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> HeaderHash calculé : {headerHashString}");

    if (headerHashString != workerHash)
    {
        Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> ERREUR : Hash incorrect. Attendu: {workerHash}, Reçu: {headerHashString}");
        throw new StratumException(StratumError.MinusOne, "bad hash");
    }

    // Vérification de la difficulté
    Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> Vérification de la difficulté");
    var headerValue = headerHash.ToBigInteger();
    var shareDiff = (double)new BigRational(CryptonoteConstants.Diff1b, headerValue);
    var stratumDifficulty = context.Difficulty;
    var ratio = shareDiff / stratumDifficulty;
    ...
    Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> ShareDiff: {shareDiff} / BlockTemplate.Difficulty {BlockTemplate.Difficulty}, StratumDifficulty: {stratumDifficulty}, Ratio: {ratio}");

    if (!isBlockCandidate && ratio < 0.99)
    {
        Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> Vérification de la difficulté avec la difficulté précédente (VarDiff)");
        ...
        if (context.VarDiff?.LastUpdate != null && context.PreviousDifficulty.HasValue)
        {
            ...
            if (ratio < 0.99)
            {
                Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> ERREUR : Share avec faible difficulté ({shareDiff})");
                throw new StratumException(StratumError.LowDifficultyShare, $"low difficulty share ({shareDiff})");
            }
    ...
    // Création du partage
    Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> Création du partage. isBlockCandidate {isBlockCandidate}");
    var result = new Share
    {
        BlockHeight = BlockTemplate.Height,
        Difficulty = stratumDifficulty,
    ...
    if (isBlockCandidate)
    {
        Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> Partage candidat pour un bloc");
        Span<byte> blockHash = stackalloc byte[32];
        ...
        Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> Bloc validé, BlockHash: {result.BlockHash}");
    }

    Console.WriteLine($"elva Debug CryptonoteJob -----> ProcessShare ---> Fin du traitement pour le worker: {worker.ConnectionId}");
    return (result, blob.ToHexString());
}
