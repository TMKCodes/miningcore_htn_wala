...
[CoinFamily(CoinFamily.Scash)]
public class ScashPool : PoolBase
{
    public ScashPool(IComponentContext ctx,
    ...
    protected ScashJobManager manager;
    private ScashTemplate coin;
    ...
    protected virtual async Task OnSubscribeAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest)
    {
        ...
        var context = connection.ContextAs<ScashWorkerContext>();
        ...
        var data = new object[]
        {
            new object[]
            {
                new object[] { ScashStratumMethods.SetDifficulty, connection.ConnectionId },
                new object[] { ScashStratumMethods.MiningNotify, connection.ConnectionId }
            }
        }
        ...
        // send intial update
        await connection.NotifyAsync(ScashStratumMethods.SetDifficulty, new object[] { context.Difficulty });
        await connection.NotifyAsync(ScashStratumMethods.MiningNotify, currentJobParams);
    }
...
// elva - suppression de la ligne en dessous
    ...
    private async Task OnConfigureMiningAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest)
    {
        ...
        if(extensions != null)
        {
            foreach(var extension in extensions)
            {
                switch(extension)
                {
                    case ScashStratumExtensions.VersionRolling:
                        ConfigureVersionRolling(connection, context, extensionParams, result);
                        break;

                    case ScashStratumExtensions.MinimumDiff:
    ...
    private void ConfigureVersionRolling(StratumConnection connection, ScashWorkerContext context,
        IReadOnlyDictionary<string, JToken> extensionParams, Dictionary<string, object> result)
    {
        //var requestedBits = extensionParams[ScashStratumExtensions.VersionRollingBits].Value<int>();
        var requestedMask = ScashConstants.VersionRollingPoolMask;

        if(extensionParams.TryGetValue(ScashStratumExtensions.VersionRollingMask, out var requestedMaskValue))
            requestedMask = uint.Parse(requestedMaskValue.Value<string>(), NumberStyles.HexNumber);

        // Compute effective mask
        context.VersionRollingMask = ScashConstants.VersionRollingPoolMask & requestedMask;

        // enabled
        result[ScashStratumExtensions.VersionRolling] = true;
        result[ScashStratumExtensions.VersionRollingMask] = context.VersionRollingMask.Value.ToStringHex8();

        logger.Info(() => $"[{connection.ConnectionId}] Using version-rolling mask {result[ScashStratumExtensions.VersionRollingMask]}");
    }

    private void ConfigureMinimumDiff(StratumConnection connection, ScashWorkerContext context,
        IReadOnlyDictionary<string, JToken> extensionParams, Dictionary<string, object> result)
    {
        var requestedDiff = extensionParams[ScashStratumExtensions.MinimumDiffValue].Value<double>();
        ...
        if(requestedDiff > poolEndpoint.Difficulty)
        {
            ...
            // enabled
            result[ScashStratumExtensions.MinimumDiff] = true;
        }
    }
...
// elva - suppression de la ligne en dessous
   //     logger.Info(() => $"Broadcasting job {((object[]) jobParams)[0]}");
    ...
    {
        var multiplier = ScashConstants.Pow2x32;
    ...
    {
        manager = ctx.Resolve<ScashJobManager>(
            new TypedParameter(typeof(IExtraNonceProvider), new ScashExtraNonceProvider(poolConfig.Id, clusterConfig.InstanceId)));
        ...
        Timestamped<JsonRpcRequest> tsRequest, CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                case ScashStratumMethods.Subscribe:
                    await OnSubscribeAsync(connection, tsRequest);
                    break;

                case ScashStratumMethods.Authorize:
                    await OnAuthorizeAsync(connection, tsRequest, ct);
                    break;

                case ScashStratumMethods.SubmitShare:
                    await OnSubmitAsync(connection, tsRequest, ct);
                    break;

                case ScashStratumMethods.SuggestDifficulty:
                    await OnSuggestDifficultyAsync(connection, tsRequest);
                    break;

                case ScashStratumMethods.MiningConfigure:
                    await OnConfigureMiningAsync(connection, tsRequest);
                    // ignored
                    break;

                case ScashStratumMethods.ExtraNonceSubscribe:
                    await connection.RespondAsync(true, request.Id);
                    break;

                case ScashStratumMethods.GetTransactions:
                    // ignored
                    break;

                case ScashStratumMethods.MiningMultiVersion:
                    // ignored
                    break;
    ...
    protected override async Task OnVarDiffUpdateAsync(StratumConnection connection, double newDiff, CancellationToken ct)
    {
        ...
        {
            await connection.NotifyAsync(ScashStratumMethods.SetDifficulty, new object[] { connection.Context.Difficulty });
            await connection.NotifyAsync(ScashStratumMethods.MiningNotify, currentJobParams);
        }
    }
