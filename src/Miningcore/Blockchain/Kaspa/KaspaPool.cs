...
// elva Utilisation du logger pour afficher le contenu
        ...
        {
     //       _logger.LogInformation($"Paramètre reçu : {param}");
         //   logger.Warn(() => $"elva --- > Param [{param}] Dropping stale share submission request (server overloaded?)");

        }

            // submit
CancellationToken cancellationToken = CancellationToken.None;; // Si `ct` est déjà défini quelque part
...
// elva - suppression de la ligne en dessous pour diminuer la quantité de log
