...
public class Cache : IEthashCache
{
    ...
    public static unsafe string GetDefaultdagDirectory()
    {
        ...
        {
            if (EthHashB3.ethash_get_default_dirname(data, chars.Length))
            {
                int length;
    ...
    public void Dispose()
    {
        if (handle != IntPtr.Zero)
        {
            ...
            {
                EthHashB3.ethash_full_delete(handle);
            }
            // Light Cache
            else
            {
                EthHashB3.ethash_light_delete(handle);
            }
                ...
                lock (genLock)
                {
                    if (!isGenerated)
                    {
                        ...
                        if (!string.IsNullOrEmpty(dagDir))
                        {
                            ...
                            handle = EthHashB3.ethash_full_new(dagDir, light, progress =>
                            {
                            ...
                            if (light != IntPtr.Zero)
                                EthHashB3.ethash_light_delete(light);
                        ...
                        else
                        {
                            ...
                            handle = EthHashB3.ethash_light_new(block);
    ...
    public unsafe bool Compute(ILogger logger, byte[] hash, ulong nonce, out byte[] mixDigest, out byte[] result)
    {
        ...
        var value = new EthHashB3.ethash_return_value();
        ...
        if (!string.IsNullOrEmpty(dagDir))
        {
            ...
            {
                EthHashB3.ethash_full_compute(handle, input, nonce, ref value);
            }
        }
        // Light Cache
        else
        {
            fixed (byte* input = hash)
            {
                EthHashB3.ethash_light_compute(handle, input, nonce, ref value);
            }
        }
        ...
        messageBus?.SendTelemetry("Ethashb3", TelemetryCategory.Hash, sw.Elapsed, value.success);
