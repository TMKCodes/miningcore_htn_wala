...
public class PROPPaymentScheme : IPayoutScheme
{
        ...
        Block block, decimal blockReward, CancellationToken ct)
    {
        ...
        foreach (var address in rewards.Keys)
        {
            ...
            if (amount > 0)
            {
                logger.Info(() => $"elva PROP - Crediting {address} with {payoutHandler.FormatAmount(amount)} for {FormatUtil.FormatQuantity(shares[address])} ({shares[address]}) shares for block {block.BlockHeight}");
