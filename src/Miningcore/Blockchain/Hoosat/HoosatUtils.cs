...
public static class HoosatUtils
{
    public static (HoosatAddressUtility, Exception) ValidateAddress(string address, string network, string coinSymbol = "HTN")
    {
        ...
        switch(network.ToLower())
        {
            case "devnet":
                ...
                break;
            case "simnet":
                networkBech32Prefix = HoosatBech32Prefix.HoosatSim;
        ...
        try
        {
            var HoosatAddressUtility = new HoosatAddressUtility(coinSymbol);
            HoosatAddressUtility.DecodeAddress(address, networkBech32Prefix);

            return (HoosatAddressUtility, null);
        }
        catch (Exception ex)
        {
            return (null, ex);
    ...
    {
        return BigInteger.Divide(HoosatConstants.Diff1Target, new BigInteger(difficulty));
    }
...
public interface HoosatIAddress
{
    string EncodeAddress();
    byte[] ScriptAddress();
    HoosatBech32Prefix Prefix();
    byte Version();
    bool IsForPrefix(HoosatBech32Prefix prefix);
}

public class HoosatAddressPublicKey : HoosatIAddress
{
    public byte version { get; private set; } = HoosatConstants.PubKeyAddrID;
    private readonly HoosatBech32Prefix prefix;
    private readonly byte[] publicKey;

    public HoosatAddressPublicKey(byte[] publicKey, HoosatBech32Prefix prefix)
    {
        if (publicKey.Length != HoosatConstants.PublicKeySize)
            throw new ArgumentException($"Public key must be {HoosatConstants.PublicKeySize} bytes", nameof(publicKey));
    ...
    {
        return HoosatBech32.Encode(prefix.ToString(), publicKey, version);
    }
    ...
    public HoosatBech32Prefix Prefix()
    {
        return prefix;
    }
    ...
    public bool IsForPrefix(HoosatBech32Prefix prefix)
    {
...
public class HoosatAddressPublicKeyECDSA : HoosatIAddress
{
    public byte version { get; private set; } = HoosatConstants.PubKeyECDSAAddrID;
    private readonly HoosatBech32Prefix prefix;
    private readonly byte[] publicKey;

    public HoosatAddressPublicKeyECDSA(byte[] publicKey, HoosatBech32Prefix prefix)
    {
        if (publicKey.Length != HoosatConstants.PublicKeySizeECDSA)
            throw new ArgumentException($"Public key must be {HoosatConstants.PublicKeySizeECDSA} bytes", nameof(publicKey));
...
public class HoosatAddressScriptHash : HoosatIAddress
{
    public byte version { get; private set; } = HoosatConstants.ScriptHashAddrID;
    private readonly HoosatBech32Prefix prefix;
    private readonly byte[] hash;

    public HoosatAddressScriptHash(byte[] serializedScript, HoosatBech32Prefix prefix)
    {
        var scriptHash = HoosatUtils.HashBlake2b(serializedScript);
        if (scriptHash.Length != HoosatConstants.Blake2bSize256)
            throw new ArgumentException($"Script hash must be {HoosatConstants.Blake2bSize256} bytes", nameof(scriptHash));
    ...
    public string EncodeAddress()
    {
        return HoosatBech32.Encode(prefix.ToString(), hash, version);
    }
...
public class HoosatAddressUtility
{
    public HoosatIAddress HoosatAddress { get; private set; }
...
//        this.CoinSymbol = coinSymbol;
    ...
    {
        { HoosatConstants.ChainPrefixMainnet, HoosatBech32Prefix.HoosatMain },
        { HoosatConstants.ChainPrefixDevnet, HoosatBech32Prefix.HoosatDev },
    ...
    {
        return HoosatBech32.Encode(PrefixToString(prefix), payload, version);
    }

    public void DecodeAddress(string addr, HoosatBech32Prefix expectedPrefix)
    {
        Console.WriteLine($"Debug HoosatJobManager -----> expectedPrefix {expectedPrefix}.");


        var (prefixString, decoded, version, error) = HoosatBech32.Decode(addr);
        if (error != null)
            throw new ArgumentException($"Decoded address is of unknown format: {error}");

        var prefix = ParsePrefix(prefixString);
        if (expectedPrefix != HoosatBech32Prefix.Unknown && expectedPrefix != prefix)
            throw new ArgumentException($"Decoded address is of wrong network. Expected {expectedPrefix.ToString()} but got {prefix.ToString()}");

        switch (version)
        {
            case HoosatConstants.PubKeyAddrID:
                this.HoosatAddress = new HoosatAddressPublicKey(decoded, prefix);

                break;
            case HoosatConstants.PubKeyECDSAAddrID:
                this.HoosatAddress = new HoosatAddressPublicKeyECDSA(decoded, prefix);

                break;
            case HoosatConstants.ScriptHashAddrID:
                this.HoosatAddress = new HoosatAddressScriptHash(HoosatUtils.HashBlake2b(decoded), prefix);
    ...
    public HoosatBech32Prefix ParsePrefix(string prefixString)
    {
    ...
    public string PrefixToString(HoosatBech32Prefix prefix)
    {
    ...
    private Dictionary<string, HoosatBech32Prefix> stringsToBech32Prefixes;
}

public static class HoosatBech32
{
